...
public class MtomImpl implements MtomTest {
    ...
    public com.fusesource.test.mtom.v1.DoMtomResponse doMtom(
    	...
    	try {
    	  ...
    	  LOG.info("Got datahandler and input stream.");

    	  // read in MTOM attachment.
    	  int bytesRead = 0;
    	  byte[] buffer = new byte[1024];
    	  while ((bytesRead = is.read(buffer)) != -1) {
    		LOG.info("Read bytes of length " + bytesRead);
    	  }
    	} catch (IOException e) {
    	...
    	DataSource source = new FileDataSource(new File("/home/cpandey/NotBackedUp/ShutterPictures/Selection_016.png"));
    	
    	LOG.info("Response DataSource created.");
